...
describe("thing shadow class unit tests", function () {
  ...
  //
  // Verify the thing shadow invokes the register callback when subscription to all
  // topics are finished. The callback is invoked based on the callback from the mqtt library.
  //
  describe("register a thing shadow to named-shadow", function () {
    var shadowDeviceConfig = {
      keyPath: 'test/data/private.pem.key',
      certPath: 'test/data/certificate.pem.crt',
      caPath: 'test/data/root-CA.crt',
      clientId: 'dummy-client-1',
    ...
    it("should trigger error when a subscription fails", function () {
      ...
      var thingShadows = thingShadow(shadowDeviceConfig, shadowConfig);
      thingShadows.register('testShadow1', { ignoreDeltas: true, persistentSubscribe: true }, function (err, granted) {
        assert.notEqual(err, null);
        for (var k = 0, grantedLen = granted.length; k < grantedLen; k++) {
          //
  ...
  //
  // Verify that named-shadow operations are performed using the correct topics
  //
  describe("Verify that MQTT operations use the correct named-shadow topics", function () {
    var clock;
    var operationTimeout = 15000;
    var shadowDeviceConfig = {
      keyPath: 'test/data/private.pem.key',
    ...
    it("should subscribe to named-shadow topics", function () {
      // Reinit mockMQTTClientObject
      mockMQTTClientObject.reInitCommandCalled();
      mockMQTTClientObject.resetPublishedMessage();
      ...
      thingShadows.on('timeout', fakeCallback);
      assert.deepEqual(
        mockMQTTClientObject.subscriptions.sort(),
        [
          '$aws/things/testShadow1/shadow/name/testShadowName/update/delta',
          '$aws/things/testShadow1/shadow/name/testShadowName/update/accepted',
          '$aws/things/testShadow1/shadow/name/testShadowName/update/rejected',
          '$aws/things/testShadow1/shadow/name/testShadowName/get/accepted',
          '$aws/things/testShadow1/shadow/name/testShadowName/get/rejected',
          '$aws/things/testShadow1/shadow/name/testShadowName/delete/accepted',
          '$aws/things/testShadow1/shadow/name/testShadowName/delete/rejected',
        ].sort()
      );
    });
  });
